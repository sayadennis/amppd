...
#SBATCH --job-name="splitnorm_v2"
#SBATCH --output=pd_project/out/splitnorm_v2_%a.out
...
splitnormalize () {
    local fn=$1
    shortn="$(cut -d'/' -f9 <<< "$fn")" # this retrieves the name "chr<num>.vcf.gz" ? 
...
splitnormalize ${din}/chr$((${SLURM_ARRAY_TASK_ID}+1)).vcf.gz
