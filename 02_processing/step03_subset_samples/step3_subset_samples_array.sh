...
#SBATCH --job-name="v2_subset"
#SBATCH --output=pd_project/out/step3_subset_samples_%a.out
...
subsetsample () {
    ...
    for subfn in $(ls ${dsampleset}/amppd_v2_sampleset_[0-9]*.txt); do # subfn="/whole/path/to/amppd_v2_sampleset_<num>.txt"
...
subsetsample "${din}/chr$((${SLURM_ARRAY_TASK_ID}+1)).splitnorm.vcf.gz"
